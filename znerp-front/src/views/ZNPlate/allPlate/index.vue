				...
				<template #default="scope">
					<div class="flex bg-blue-50 py-1 px-3 dark:bg-blue-900">
						...
						<div class="pl-4">
							...
							<template v-if="scope.row.type == 1 && scope.row.orderItems[0].printers.length === 0" >
								<el-divider direction="vertical" />
							...
							<template v-if="scope.row.type == 1 && scope.row.orderItems[0].printers.length > 0" >
								<el-divider direction="vertical" />
								<el-button @click="uploadHandle('plate', scope.row.orderItems[0])" link  v-if="scope.row.orderItems[0].printers[0].userId === userID">
									{{ scope.row.orderItems[0].printers[0].imgUrl ? '修改' : '上传' }}图片
								</el-button>

									<template v-if="scope.row.orderItems[0].printers[0].imgUrl">
										<el-divider direction="vertical" />
										<el-button  :type="scope.row.orderItems[0].printStatus == 1 ? 'success' : scope.row.orderItems[0].printStatus == 2 ? 'danger' : 'primary'" @click="checkHandle('plate', scope.row.orderItems[0].printers[0])" link>
											审核{{ scope.row.orderItems[0].printStatus == 1 ? '通过' : scope.row.orderItems[0].printStatus == 2 ? '不通过' : ''}}
										</el-button>
									</template>
							</template>
						</div>
					...
					:preview-src-list="scopeChild.row.upload_img ? [scopeChild.row.upload_img] : [noPic]" preview-teleported fit="cover" />
						...
						<el-table-column v-if="scope.row.type == 2"  width="200">
							<template #default="scopeChild">
								...
								<el-button v-if="scopeChild.row.printers.length === 0" type="primary" link @click="handleUpdateStatus(scopeChild.row)">
									领取任务
								</el-button>
								<template v-else>
									<template v-if="scopeChild.row.printers[0].imgUrl">
										<el-button :type="scopeChild.row.printStatus == 1 ? 'success' : scopeChild.row.printStatus == 2 ? 'danger' : 'primary'" @click="checkHandle('plate', scopeChild.row)" link>
											审核{{ scopeChild.row.printStatus == 1 ? '通过' : scopeChild.row.printStatus == 2 ? '不通过' : ''}}
										</el-button>
										<el-divider direction="vertical"  v-if="scopeChild.row.printStatus == 2"/>
									</template>
									<el-button type="primary" link  v-if="scopeChild.row.printStatus !== 1 && scopeChild.row.printers[0].userId === userID" @click="uploadHandle('plate', scopeChild.row)">
										{{ scopeChild.row.printers[0].imgUrl ? '修改' : '上传' }}图片
									</el-button>
								</template>
							</div>
							</template>
...
const handleUpdateStatus = async (info: any) => {
	try {
		// 领取任务二次确认
		await message.confirm(`确定领取制版任务吗？`)
		// 领取任务
		let params = {
