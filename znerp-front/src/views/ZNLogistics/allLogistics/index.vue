	...
	<ContentWrap>
		...
		<el-form class="-mb-15px" :model="queryParams" ref="queryFormRef" :inline="true" label-width="88px">
			<el-form-item label="物流名称" prop="name">
				<el-input
v-model="queryParams.name" placeholder="请输入物流名称" clearable @keyup.enter="handleQuery"
					class="!w-240px" />
			</el-form-item>
			<el-form-item>
				...
				<el-button type="primary" plain @click="openForm('create')">
					<Icon icon="ep:plus" class="mr-5px" /> 新增物流渠道
				</el-button>
			</el-form-item>
		</el-form>		
	</ContentWrap>	
	...
	<ContentWrap>
		<el-table v-loading="loading" :data="list" :stripe="true">
			<el-table-column align="center" label="物流商名称" min-width="100" prop="name" />
			<el-table-column align="center" label="物流商代码" prop="code" />
			<el-table-column align="left" label="授权状态" prop="authorization">
				<template #default="scope">
					<el-tag :type="scope.row.authorizeStatus == 0 ? 'info': 'success'">{{ scope.row.authorizeStatus == 0 ? '未授权' : '已授权' }}</el-tag>
				</template>
			</el-table-column>
...
/** ERP 物流管理 */
defineOptions({ name: 'ZnErpLogistics' })
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await ZnerpApi.deleteLogistics(id)
...
const handleActive = async (type: string, id: number) => {
  try {
		let params = { 
			id: id,
			status: type === 'active' ? false : true
		}
    await ZnerpApi.logisticsUpdate(params)
