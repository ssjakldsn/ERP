	...
	<Dialog v-model="dialogVisible" :title="dialogTitle">
		<el-form ref="formRef" v-loading="formLoading" :model="formData" :rules="formRules" label-width="120px">
			...
			<el-form-item label="物流商代码" prop="code">
				<el-input v-model="formData.code" placeholder="请输入物流商代码" />
			</el-form-item>
			<el-form-item label="apikey" prop="apikey">
				<el-input v-model="formData.apikey" placeholder="请输入Key" />
			</el-form-item>
...
const formRules = reactive({
	name: [{ required: true, message: '物流商名称不能为空', trigger: 'blur' }],
	code: [{ required: true, message: '物流商代码不能为空', trigger: 'blur' }],
...
const open = async (type: string, info: any) => {
	...
	if (info) {
		...
		try {
			formData.value = {
				id: info.id,
				name: info.name, // 物流渠道名称,
				code: info.code, // 物流商代码
				apikey: info.apikey, // apikey
...
const submitForm = async () => {
	...
	try {
		if(formType.value === 'create') {
			await ZnerpApi.logisticsCreate(formData.value)
			message.success( t('common.createSuccess'))
		} else {
			await ZnerpApi.logisticsUpdate(formData.value)
