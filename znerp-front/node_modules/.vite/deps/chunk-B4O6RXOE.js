  ...
  var api = {
    ...
    formData: function formData3(fields) {
      return tidyFields(fields).reduce(function(initial, id2) {
        ...
        return initial;
      }, h2.options.appendValue !== false ? copy(h2.appendData) : {});
    },
    ...
    fetch: function fetch3(opt) {
      return new Promise(function(resolve, reject) {
        ...
        h2.beforeFetch(opt).then(function() {
          return asyncFetch(opt, h2.fc.create.fetch).then(resolve)["catch"](reject);
        });
      });
    },
    watchFetch: function watchFetch(opt, callback, error) {
      return h2.fc.watchLoadData(function(get, change) {
        ...
        h2.beforeFetch(_opt).then(function() {
          return asyncFetch(_opt, h2.fc.create.fetch).then(function(res) {
    ...
    setData: function setData(id2, data5) {
      return h2.fc.setData(id2, data5);
    },
    ...
    renderCtx: function renderCtx(ctx, parent) {
      ...
      try {
        ...
        if (!this.cache[ctx.id] || this.cache[ctx.id].slot !== rule.slot) {
          ...
          this.ctxProp(ctx);
          var prop = ctx.prop;
          prop.preview = !!(prop.preview != null ? prop.preview : this.vm.props.preview !== void 0 ? this.vm.props.preview : this.options.preview || false);
  ...
  extend(Handler2.prototype, {
    ...
    watchCtx: function watchCtx(ctx) {
      ...
      this.bus.$once("load-end", function() {
        var computed2 = ctx.rule.computed;
        if (!computed2) {
          return;
        }
        if (_typeof(computed2) !== "object") {
          computed2 = {
            value: computed2
          };
        }
        Object.keys(computed2).forEach(function(k) {
          ctx.watch.push(watch(function() {
            var item = computed2[k];
            if (!item)
              return void 0;
            var fn;
            if (_typeof(item) === "object") {
            ...
            immediate: k === "value" ? [void 0, null, ""].indexOf(ctx.rule.value) > -1 : true
          }));
        });
      });
...
var loadData = function loadData2(fc) {
    ...
    mounted: function mounted7(inject2, rule, api) {
      ...
      attrs2.forEach(function(attr) {
        if (attr && (attr.attr || attr.template)) {
          var unwatch2 = fc.watchLoadData(debounce(function(get) {
            var value;
            if (attr.template) {
  ...
  loadData3.watch = loadData3.created;
  return loadData3;
};
...
function FormCreateFactory(config) {
  ...
  function register() {
    var data5 = nameProp.apply(void 0, arguments);
    if (data5.id && data5.prop)
      providers[data5.id] = _objectSpread2(_objectSpread2({}, data5.prop), {}, {
        name: data5.id
      });
  }
  ...
  extend(FormCreate2.prototype, {
    ...
    watchLoadData: function watchLoadData(fn) {
      ...
      var get = function get2(id2, def) {
        ...
        var val = _this6.getLoadData(id2, def);
        var split = id2.split(".");
        var key = split.shift();
        ...
        var callback = debounce(function() {
          if (key !== id2) {
            ...
            if (JSON.stringify(temp) !== JSON.stringify(unwatch2[id2].val)) {
  ...
  setPrototypeOf(FcComponent, create2);
  Object.defineProperties(FcComponent, {
    fetch: {
      get: function get() {
        return create2.fetch;
      },
...
var manager = {
  ...
  update: function update4() {
    ...
    this.rule = {
      ...
      "class": [form2.className, form2["class"], "form-create", this.options.preview ? "is-preview" : ""],
      style: form2.style,
      type: "form"
    };
  },
  ...
  makeInfo: function makeInfo(rule, uni, ctx) {
    ...
    }) : (titleProp.title || "") + (form2.labelSuffix || form2["label-suffix"] || "")];
...
@form-create/element-ui/dist/form-create.esm.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-B4O6RXOE.js.map
